...
class DPyramid : public DDrawableStaticBase< DPyramid >
{
public:
	DPyramid( MXGraphics& Graphics, std::mt19937& Rng, std::uniform_real_distribution< float >& AngleRand, std::uniform_real_distribution< float >& SpeedRand,
		std::uniform_real_distribution< float >& OrbitRand, std::uniform_real_distribution< float >& RadiusRand );
	DPyramid( DDrawable const& ) = delete;
	~DPyramid() = default;
